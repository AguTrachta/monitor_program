...
void update_disk_metrics()
{
    ...
    // Convertir las operaciones de lectura y escritura a miles (dividir entre 1000)
    double reads_in_thousands = stats.reads / 1000.0;
    double writes_in_thousands = stats.writes / 1000.0;
    // Actualizar las métricas de lectura, escritura y tiempo dedicado en Prometheus
    pthread_mutex_lock(&lock);
...
{
    static NetStats prev_stats = {0, 0, 0, 0}; // Para almacenar los valores previos
    NetStats current_stats = get_network_stats("wlp2s0");

    // Calcular el ancho de banda (en bytes por segundo)
    double bandwidth_rx = (double)(current_stats.bytes_received - prev_stats.bytes_received);
    double bandwidth_tx = (double)(current_stats.bytes_transmitted - prev_stats.bytes_transmitted);

    // Calcular la relación de paquetes
    double packet_ratio = 0;
    if (current_stats.packets_received > 0)
    {
        packet_ratio = (double)current_stats.packets_transmitted / (double)current_stats.packets_received;
    }

    pthread_mutex_lock(&lock);
    prom_gauge_set(network_bandwidth_rx_metric, bandwidth_rx, NULL); // Ancho de banda en recepción
    prom_gauge_set(network_bandwidth_tx_metric, bandwidth_tx, NULL); // Ancho de banda en transmisión
    prom_gauge_set(network_packet_ratio_metric, packet_ratio, NULL); // Relación de paquetes
    pthread_mutex_unlock(&lock);
...
void init_disk_metrics()
{
    ...
    disk_reads_metric =
        prom_gauge_new("disk_reads_operations", "Número de operaciones de lectura en el disco (en miles)", 0, NULL);
    if (disk_reads_metric == NULL)
    {
        fprintf(stderr, "Error al crear la métrica de operaciones de lectura de disco\n");
    }

    // Registramos la métrica de lectura de disco
    if (prom_collector_registry_must_register_metric(disk_reads_metric) != 0)
    {
        fprintf(stderr, "Error al registrar las métricas de operaciones de lectura de disco\n");
    }

    // Creamos la métrica para operaciones de escritura en disco
    disk_writes_metric =
        prom_gauge_new("disk_writes_operations", "Número de operaciones de escritura en el disco (en miles)", 0, NULL);
    if (disk_writes_metric == NULL)
    {
        fprintf(stderr, "Error al crear la métrica de operaciones de escritura de disco\n");
    }

    // Registramos la métrica de escritura de disco
    if (prom_collector_registry_must_register_metric(disk_writes_metric) != 0)
    {
        fprintf(stderr, "Error al registrar las métricas de operaciones de escritura de disco\n");
    }

    // Creamos la métrica para el tiempo dedicado a operaciones de lectura
    disk_read_time_metric =
        prom_gauge_new("disk_read_time", "Tiempo dedicado a operaciones de lectura (segundos)", 0, NULL);
    if (disk_read_time_metric == NULL)
    {
        fprintf(stderr, "Error al crear la métrica de tiempo de lectura de disco\n");
    }

    // Registramos la métrica de tiempo de lectura de disco
    if (prom_collector_registry_must_register_metric(disk_read_time_metric) != 0)
    {
        fprintf(stderr, "Error al registrar las métricas de tiempo de lectura de disco\n");
    }

    // Creamos la métrica para el tiempo dedicado a operaciones de escritura
    disk_write_time_metric =
        prom_gauge_new("disk_write_time", "Tiempo dedicado a operaciones de escritura (segundos)", 0, NULL);
    if (disk_write_time_metric == NULL)
    {
        fprintf(stderr, "Error al crear la métrica de tiempo de escritura de disco\n");
    }

    // Registramos la métrica de tiempo de escritura de disco
    if (prom_collector_registry_must_register_metric(disk_write_time_metric) != 0)
    {
        fprintf(stderr, "Error al registrar las métricas de tiempo de escritura de disco\n");
    }
}
...
void init_network_metrics()
{
    ...
    network_bandwidth_rx_metric =
        prom_gauge_new("network_bandwidth_receive", "Ancho de banda de recepción (bytes/segundo)", 0, NULL);
    network_bandwidth_tx_metric =
        prom_gauge_new("network_bandwidth_transmit", "Ancho de banda de transmisión (bytes/segundo)", 0, NULL);
    network_packet_ratio_metric =
        prom_gauge_new("network_packet_ratio", "Relación de paquetes transmitidos/recibidos", 0, NULL);
...
void init_count_processes()
{
    ...
    count_processes_metric = prom_gauge_new("running_processes_count", "Número de procesos en ejecución", 0, NULL);
    ...
    if (prom_collector_registry_must_register_metric(count_processes_metric) != 0)
    {
...
void init_context_switches_metric()
{
    ...
    context_switches_metric = prom_gauge_new("context_switches_total", "Número total de cambios de contexto", 0, NULL);
